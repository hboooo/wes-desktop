...
namespace Wes.Launcher.Util
{
    ...
    public class FtpUtil
    {
        ...
        #region 构造函数
        public FtpUtil(Uri uri, string username, string password)
        {
            this.Uri = uri;
        ...
        public delegate void FtpUploadFinish(string second);
            ...
            try
            {
                ...
                using (FileStream fs = fileInf.OpenRead())
                {
                    using (Stream stream = request.GetRequestStream())
                    {
                        ...
                        sw.Stop();
                        FtpUploadFinishEvent?.Invoke(sw.Elapsed.ToString());
                    }
                }
        ...
        /// <summary>
        ///从FTP服务指定文件夹下载所有文件到本地（比较最新时间）
        /// </summary>
        /// <param name="ftpDirecotry"></param>
        /// <param name="localDirecotry"></param>
        /// <returns></returns>
        public void DownDirectoryNewFile(List<FileStruct> lstFile, string ftpDirecotry, string localDirecotry)
        {
            try
            {
                //写入Ftp文件列表到本地
                foreach (FileStruct file in lstFile)
                {
                    if (file.IsDirectory)
                        continue;
                    string localFilePath = localDirecotry + file.Name;
                    string localDirector = Path.GetDirectoryName(localFilePath);
                    string extension = Path.GetExtension(localFilePath);

                    if (!extension.Equals(".btw", StringComparison.OrdinalIgnoreCase) &&
                        !extension.Equals(".tff", StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }
                    if (!Directory.Exists(localDirector))
                    ...
                    if (File.Exists(localFilePath)) //本地已经存在该文件
                    {
                        //DateTime dtLocal = File.GetCreationTime(localFilePath);
                        var dtLocal = new FileInfo(localFilePath).LastWriteTime;
                        int compareValue = dtLocal.CompareTo(file.UpdateTime);
                        if (compareValue > 0) //不是最新的
                            continue;
                    }
                    byte[] buffer = new byte[2048];
                    int bytesCount = 0;
                    FtpWebResponse response = CreateResponse(new Uri(this.Uri.ToString() + ftpDirecotry + "/" + file.Name),
