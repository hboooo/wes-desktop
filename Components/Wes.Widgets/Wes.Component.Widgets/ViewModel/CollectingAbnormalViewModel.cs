...
{
    public class CollectingAbnormalViewModel : ScanViewModelBase<WesFlowID, CollectingAbnormalAction>, IViewModel
    {
        public ICommand PartNoDetailsCommand { get; private set; }

        public ICommand BtnSearchCommand { get; private set; }

        public CollectingAbnormalViewModel()
        {
            this.SelfInfo.rcv = "";
            this.SelfInfo.doneList = new List<object>();
        }

        protected override CollectingAbnormalAction GetAction()
        {
            var gatherAbnormalAction = ViewModelFactory.CreateActoin<CollectingAbnormalAction>() as CollectingAbnormalAction;
            if (gatherAbnormalAction != null)
            {
                PartNoDetailsCommand = new RelayCommand<dynamic>(gatherAbnormalAction.GetGetherPortNoDetails);
                BtnSearchCommand = new RelayCommand(gatherAbnormalAction.SearchCommand);
            }
